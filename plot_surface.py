...
def email_sender(result, config):
    ...
    # 163用户名
    mail_user = '13220102191'
    # 密码(部分邮箱为授权码)
    mail_pass = 'KNJNHCRERARMJRXF'
    # 邮件发送方邮箱地址
    sender = '13220102191@163.com'
    # 邮件接受方邮箱地址，注意需要[]包裹，这意味着你可以写多个邮件地址群发
    receivers = ['470651748@qq.com']

    # 设置email信息
    # 邮件内容设置
    msg = '/***** \n EXP: {} \n Result: {} \n GPU: {} \n User: {} \n Config: {} \n *****/ \n'.format(
        config.exp, result, config.gpu, getpass.getuser(), config
    )
    message = MIMEText(msg, 'plain', 'utf-8'
    )
    # 邮件主题
    message['Subject'] = 'EXP_{}_Done'.format(config.exp)
    # 发送方信息
    message['From'] = sender
    # 接受方信息
...
if __name__ == '__main__':
    ...
    # send emails
    #--------------------------------------------------------------------------
    email_sender(result='{}-loss-landscape done'.format(args.model), config=args)
